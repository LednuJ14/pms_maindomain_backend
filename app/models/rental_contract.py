...
class RentalContract(db.Model):
    ...
    # Relationships
    tenant_unit_id = db.Column(db.Integer, db.ForeignKey('tenant_units.id'), nullable=True)
    ...
    def __init__(self, tenant_id, unit_id, property_id, contract_type, start_date, 
                 monthly_rent, tenant_unit_id=None, security_deposit=None, end_date=None, **kwargs):
    ...
    def activate(self):
        ...
        # Send automated message with subdomain link to the inquiry chat
        try:
            if self.inquiry_id:
                ...
                if property_obj and property_obj.portal_enabled and property_obj.portal_subdomain:
                    # Construct portal URL (using localhost for development, can be env variable in production)
                    import os
                    base_domain = os.environ.get('FRONTEND_DOMAIN', 'localhost:3000')
                    protocol = 'https' if 'localhost' not in base_domain else 'http'
                    portal_url = f"{protocol}://{property_obj.portal_subdomain}.{base_domain}"
                    
                    message_text = (
                        f"Congratulations! Both parties have signed the contract and it is now ACTIVE. 🎉\n\n"
                        f"You can now access your dedicated Tenant Portal to manage your unit, pay bills, and submit maintenance requests.\n\n"
                        f"Tenant Portal Link: {portal_url}\n\n"
                        f"Please save this link for future reference!"
                    )
                    
                    # Assume landlord_signed_by holds the property manager's user ID
                    manager_id = self.landlord_signed_by
                    if manager_id:
            ...
            from flask import current_app
            current_app.logger.error(f"Failed to send automated activation message: {str(e)}")
